...
function problemA() {
  ...
  Promise.all([
    promisifiedReadFile("poem-two/stanza-01.txt").then((stanza1) => {
      blue(stanza1);
    }),
    promisifiedReadFile("poem-two/stanza-02.txt").then((stanza2) => {
      blue(stanza2);
    }),
...
function problemB() {
  ...
  Promise.all(
    filenames.map((ele) =>
      promisifiedReadFile(ele).then((stanza) => blue(stanza))
    )
  ).finally(() => console.log("done"));
}
